...
resource "aws_launch_template" "web" {
  name                   = "WebServer-HA-${var.bg_indicator}-LT"
...
resource "aws_autoscaling_group" "web" {
  name                = "WebServer-HA-${var.bg_indicator}-ASG-Ver-${aws_launch_template.web.latest_version}"
  min_size            = 2
  max_size            = 2
  min_elb_capacity    = 2
  ...
  dynamic "tag" {
    for_each = {
      Name   = "WebServer in ${var.bg_indicator}-ASG-v${aws_launch_template.web.latest_version}"
      TAGKEY = "TAGVALUE"
    }
    content {
      key                 = tag.key
...
#-------------------------------------------------------------------------------
resource "aws_lb" "web" {
  name               = "WebServer-HA-${var.bg_indicator}-ALB"
...
resource "aws_lb_target_group" "web" {
  name                 = "WebServer-HA-${var.bg_indicator}-TG"
  vpc_id               = aws_default_vpc.default.id
  port                 = 80
  protocol             = "HTTP"
